...
export interface ItemCustomConfig {
...
export const SearchResultsList: React.FC<SearchResultsListProps> = ({
  ...
  const [globalConfig, setGlobalConfig] = useState<ItemCustomConfig>({
    downloadType: 'video',
  ...
  const [itemConfigs, setItemConfigs] = useState<Record<string, ItemCustomConfig>>(() => {
    const initial: Record<string, ItemCustomConfig> = {};
  ...
  // Apply global config to ALL items or ONLY SELECTED items
  const handleApplyGlobalToAll = () => {
    const updated: Record<string, ItemCustomConfig> = {};
    allEntries.forEach((e) => {
      updated[e.id] = {
        ...globalConfig,
    ...
    toast.success('Đã đồng bộ cài đặt chung này cho toàn bộ video trong danh sách!');
  };

  // Update a single item's config
  const updateItemConfig = (id: string, partial: Partial<ItemCustomConfig>) => {
    setItemConfigs((prev) => {
      const existing = prev[id] || globalConfig;
      return {
        ...prev,
        [id]: { ...existing, ...partial },
      };
  ...
  const createDownloadRequest = (entry: PlaylistEntry, config: ItemCustomConfig): DownloadRequest => {
  ...
  const handleDownloadSingle = (entry: PlaylistEntry, overrideType?: 'video' | 'audio', overrideQualityOrFormat?: string) => {
    ...
    const currentConfig = { ...(itemConfigs[entry.id] || globalConfig) };
    if (overrideType) currentConfig.downloadType = overrideType;
    if (overrideQualityOrFormat) {
      if (overrideType === 'video') currentConfig.quality = overrideQualityOrFormat;
      if (overrideType === 'audio') currentConfig.audioFormat = overrideQualityOrFormat;
    }

    const req = createDownloadRequest(entry, currentConfig);
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-white/[0.08]">
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] text-xs font-semibold text-slate-300 hover:text-white transition-all cursor-pointer shrink-0 self-start md:self-auto"
        >
      ...
      {/* 1. GLOBAL SETTINGS PANEL (Áp dụng cho toàn bộ video) */}
      <AnimatePresence>
        {isGlobalSettingExpanded && (
          ...
          >
            <div className="p-4 rounded-2xl bg-gradient-to-br from-indigo-950/40 via-purple-950/30 to-slate-900/50 border border-indigo-500/30 shadow-lg space-y-3.5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/[0.06] pb-2.5">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold transition-all shadow-md cursor-pointer self-start sm:self-auto"
                  title="Áp dụng cài đặt này ngay cho tất cả video"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                ...
                <div className="space-y-1">
                  <label className="text-[11px] font-bold text-slate-400">Định dạng tải:</label>
                  <div className="grid grid-cols-2 gap-1.5">
                    <button
                      onClick={() => setGlobalConfig((prev) => ({ ...prev, downloadType: 'video' }))}
                      className={`flex items-center justify-center gap-1.5 py-1.5 rounded-xl font-bold transition-all cursor-pointer ${
                        globalConfig.downloadType === 'video'
                    ...
                    <button
                      onClick={() => setGlobalConfig((prev) => ({ ...prev, downloadType: 'audio' }))}
                      className={`flex items-center justify-center gap-1.5 py-1.5 rounded-xl font-bold transition-all cursor-pointer ${
                        globalConfig.downloadType === 'audio'
                ...
                <div className="space-y-1">
                  <label className="text-[11px] font-bold text-slate-400">
                    {globalConfig.downloadType === 'video' ? 'Độ phân giải:' : 'Định dạng âm thanh:'}
                  </label>
                  {globalConfig.downloadType === 'video' ? (
                    <select
                      value={globalConfig.quality}
                      onChange={(e) => setGlobalConfig((prev) => ({ ...prev, quality: e.target.value }))}
                      className="w-full bg-slate-900 border border-white/10 text-slate-200 font-semibold rounded-xl py-2 px-3 focus:outline-none focus:border-indigo-500/50"
                    >
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      <select
                        value={globalConfig.audioFormat}
                        onChange={(e) => setGlobalConfig((prev) => ({ ...prev, audioFormat: e.target.value }))}
                        className="bg-slate-900 border border-white/10 text-slate-200 font-semibold rounded-xl py-2 px-2 focus:outline-none focus:border-pink-500/50"
                      >
                      ...
                      <select
                        value={globalConfig.audioQuality}
                        onChange={(e) => setGlobalConfig((prev) => ({ ...prev, audioQuality: e.target.value }))}
                        className="bg-slate-900 border border-white/10 text-slate-200 font-semibold rounded-xl py-2 px-2 focus:outline-none focus:border-pink-500/50"
                      >
                        <option value="320K">320k (Cao nhất)</option>
                ...
                {/* Subtitles & Container */}
                <div className="space-y-1">
                  <label className="text-[11px] font-bold text-slate-400">Định dạng file chứa:</label>
                  <select
                    value={globalConfig.videoContainer}
                    onChange={(e) => setGlobalConfig((prev) => ({ ...prev, videoContainer: e.target.value }))}
                    className="w-full bg-slate-900 border border-white/10 text-slate-200 font-semibold rounded-xl py-2 px-3 focus:outline-none focus:border-indigo-500/50"
                  >
                    <option value="mp4">MP4 (Phổ biến nhất)</option>
                    <option value="mkv">MKV (Hỗ trợ đa phụ đề)</option>
                ...
                {/* Toggles */}
                <div className="space-y-1">
                  <label className="text-[11px] font-bold text-slate-400">Tùy chọn bổ sung:</label>
                  <div className="flex flex-wrap gap-2 pt-0.5">
                    <label className="flex items-center gap-1.5 text-[11px] text-slate-300 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={globalConfig.embedMetadata}
                        onChange={(e) => setGlobalConfig((prev) => ({ ...prev, embedMetadata: e.target.checked }))}
                    ...
                    <label className="flex items-center gap-1.5 text-[11px] text-slate-300 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={globalConfig.embedThumbnail}
                        onChange={(e) => setGlobalConfig((prev) => ({ ...prev, embedThumbnail: e.target.checked }))}
                        className="rounded accent-indigo-500"
                      />
                      <span>Thumbnail</span>
                    </label>
                    <label className="flex items-center gap-1.5 text-[11px] text-slate-300 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={globalConfig.embedSubtitles}
                        onChange={(e) => setGlobalConfig((prev) => ({ ...prev, embedSubtitles: e.target.checked }))}
                        className="rounded accent-indigo-500"
                      />
                      <span>Phụ đề</span>
                    </label>
        ...
        {filteredEntries.map((entry, idx) => {
          ...
          return (
            ...
            >
                ...
                <div className="flex items-center gap-2 shrink-0 self-end md:self-center flex-wrap">
                  ...
                  <div className="flex items-center gap-1 bg-slate-900/90 p-1 rounded-xl border border-white/10 text-xs">
                    <select
                      ...
                      onChange={(e) => updateItemConfig(entry.id, { downloadType: e.target.value as any })}
                    ...
                    {itemCfg.downloadType === 'video' ? (
                      <select
                        ...
                        onChange={(e) => updateItemConfig(entry.id, { quality: e.target.value })}
                      ...
                      <select
                        ...
                        onChange={(e) => updateItemConfig(entry.id, { audioFormat: e.target.value })}
                  ...
                  <button
                    ...
                    title="Mở tùy chỉnh chi tiết tên file, cắt đoạn, phụ đề cho video này"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:opacity-90 text-white text-[11px] font-bold transition-all cursor-pointer shadow-md shadow-indigo-600/20"
              ...
              {/* Accordion: Per-Video Detailed Settings (Tên file, Cắt thời gian, Định dạng chi tiết) */}
              <AnimatePresence>
                {isExpanded && (
                  <motion.div
                    initial={{ opacity: 0, height: 0 }}
                    ...
                    className="overflow-hidden border-t border-white/[0.06] bg-slate-950/60 p-3.5 rounded-b-2xl space-y-3"
                  >
                    <div className="flex items-center gap-2 text-xs font-bold text-indigo-300">
                      <FileEdit className="w-3.5 h-3.5" />
                      <span>Tùy chỉnh riêng cho: #{idx + 1} - {entry.title}</span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                      ...
                      <div className="space-y-1 sm:col-span-2">
                        <label className="text-[11px] font-semibold text-slate-400 flex items-center gap-1">
                          ...
                          Đổi tên file lưu trữ (Tùy chọn):
                        </label>
                        <input
                          type="text"
                          value={itemCfg.customFilename}
                          onChange={(e) => updateItemConfig(entry.id, { customFilename: e.target.value })}
                          placeholder={entry.title}
                          className="w-full bg-slate-900 border border-white/10 rounded-xl px-3 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-indigo-500 font-mono"
                        />
                      </div>

                      {/* Video Container / Audio Bitrate */}
                      <div className="space-y-1">
                        <label className="text-[11px] font-semibold text-slate-400">
                          {itemCfg.downloadType === 'video' ? 'Container định dạng:' : 'Chất lượng Bitrate:'}
                        </label>
                        {itemCfg.downloadType === 'video' ? (
                          <select
                            value={itemCfg.videoContainer}
                            onChange={(e) => updateItemConfig(entry.id, { videoContainer: e.target.value })}
                            className="w-full bg-slate-900 border border-white/10 rounded-xl px-2.5 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-indigo-500"
                          >
                          ...
                          <select
                            value={itemCfg.audioQuality}
                            onChange={(e) => updateItemConfig(entry.id, { audioQuality: e.target.value })}
                            className="w-full bg-slate-900 border border-white/10 rounded-xl px-2.5 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-pink-500"
                          >
                      ...
                      <div className="space-y-1 sm:col-span-2">
                        <label className="text-[11px] font-semibold text-slate-400 flex items-center gap-1">
                          ...
                          Cắt đoạn video / nhạc (Thời gian mm:ss hoặc hh:mm:ss):
                        </label>
                        <div className="flex items-center gap-2">
                          <input
                            type="text"
                            value={itemCfg.trimStart}
                            onChange={(e) => updateItemConfig(entry.id, { trimStart: e.target.value })}
                            placeholder="Từ: 00:00"
                            className="w-1/2 bg-slate-900 border border-white/10 rounded-xl px-2.5 py-1.5 text-xs text-slate-200 font-mono text-center focus:outline-none focus:border-amber-500"
                          />
                          <span className="text-slate-500">đến</span>
                          <input
                            type="text"
                            value={itemCfg.trimEnd}
                            onChange={(e) => updateItemConfig(entry.id, { trimEnd: e.target.value })}
                            placeholder="Đến: 03:45"
                            className="w-1/2 bg-slate-900 border border-white/10 rounded-xl px-2.5 py-1.5 text-xs text-slate-200 font-mono text-center focus:outline-none focus:border-amber-500"
                          />
                        </div>
                      </div>
                      ...
                      <div className="space-y-1 flex flex-col justify-end">
                        <div className="flex items-center gap-3 pt-2">
                          <label className="flex items-center gap-1.5 text-[11px] text-slate-300 cursor-pointer">
                            <input
                              type="checkbox"
                              checked={itemCfg.embedSubtitles}
                              onChange={(e) => updateItemConfig(entry.id, { embedSubtitles: e.target.checked })}
                              className="rounded accent-indigo-500"
                            />
                          ...
                          <label className="flex items-center gap-1.5 text-[11px] text-slate-300 cursor-pointer">
                            <input
                              type="checkbox"
                              checked={itemCfg.sponsorblock}
                              onChange={(e) => updateItemConfig(entry.id, { sponsorblock: e.target.checked })}
                              className="rounded accent-indigo-500"
                            />
                            <span>Bỏ QC</span>
                          </label>
