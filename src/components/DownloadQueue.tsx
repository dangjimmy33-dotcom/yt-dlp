...
import { Trash2, Inbox, AlertTriangle, CheckCircle2 } from "lucide-react";
...
export const DownloadQueue: React.FC<DownloadQueueProps> = ({
  ...
  const failedOrCancelledTasks = tasks.filter(
    (t) => t.status === "error" || t.status === "cancelled"
  );

  const filteredTasks = tasks.filter((t) => {
    if (filter === "active")
      return t.status === "downloading" || t.status === "merging" || t.status === "queued";
    if (filter === "completed") return t.status === "completed";
    return true;
  });

  const isDownloadingAny = activeTasks.length > 0;
  const hasInactiveTasks = tasks.some(
    (t) => t.status !== "downloading" && t.status !== "merging" && t.status !== "queued"
  );
  ...
  return (
    <div className="w-full space-y-4">
      ...
      {/* Header with Filter Pills and Clear Buttons Toolbar */}
      <div className="flex flex-wrap items-center justify-between gap-3 px-1">
        ...
        {/* Action Cleanup Buttons */}
        <div className="flex items-center gap-2 flex-wrap">
          {/* Clear Failed / Cancelled */}
          {failedOrCancelledTasks.length > 0 && onClearFailedOrCancelled && (
            <button
              onClick={onClearFailedOrCancelled}
              ...
              title="Xóa toàn bộ các mục bị lỗi và đã hủy"
            >
              <AlertTriangle className="w-3.5 h-3.5" />
              <span>Dọn mục lỗi ({failedOrCancelledTasks.length})</span>
            </button>
          )}
          ...
          {completedTasks.length > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 border border-emerald-500/20 text-xs font-semibold transition-all cursor-pointer"
              title="Xóa toàn bộ các mục đã tải xong"
            >
              <CheckCircle2 className="w-3.5 h-3.5" />
              <span>Dọn hoàn thành ({completedTasks.length})</span>
            </button>
          )}

          {/* Clear All Inactive */}
          {hasInactiveTasks && onClearAll && (
            <button
              onClick={onClearAll}
              ...
              title="Dọn dẹp toàn bộ hàng đợi"
            >
              <Trash2 className="w-3.5 h-3.5 text-slate-400" />
              <span>Dọn tất cả</span>
            </button>
          )}
        </div>
      </div>
        ...
        <AnimatePresence mode="popLayout">
          {filteredTasks.length > 0 ? (
            ...
            >
              ...
              <div>
                <h4 className="text-sm font-bold text-slate-200">Hàng đợi trống</h4>
